...
$$
类似地，记 $h(s) := {\mathscr H}_n[g(\rho)]$，可以证明 $h(s)$ 与 $f(r)$ 为同一函数（根据 **\[Thm 2.5.3\]** 的正交性），因此也将其记为逆变换，即
$$
...
$$
记频域函数 $F(\rho, \varphi)$ 表示对 $f(r, \theta)$ 做傅里叶变换后的结果，化简后得到
$$
\begin{aligned}
...
&= \sum_{s=1}^{+\infty} c_n\Bigl( \frac{\lambda_{ns}}{\alpha} \Bigr) \frac{2}{\alpha^2 J_{n+1}^2(\lambda_{ns})} \int_{0}^{\infty} \rho J_n\Bigl( \frac{\lambda_{ns}}{\alpha} \rho \Bigr)
