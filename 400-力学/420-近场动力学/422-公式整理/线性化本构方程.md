...
* 近场动力学模型
  $$
  W_{\mathrm{pd}}({\boldsymbol x}) = \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) \xi^{-2} {\boldsymbol \eta}^{\mathrm T} ({\boldsymbol \xi} \otimes {\boldsymbol \xi}) {\boldsymbol \eta} {{\mathrm d\!} V_{{\boldsymbol x}'}}
  $$
...
* 可以类似的认为任意物质点所连接的每根键的变形，储存了此时的变形状态下的弹性势能。所以可以写出该模型下的弹性能密度（在没有断裂时，应当与连续介质力学的应变能密度等价）表达式，如下式所示
$$
\begin{aligned}
W_{\mathrm{pd}}({\boldsymbol x}) &= \frac12 \int_{\mathcal H_\delta({\boldsymbol x})} \frac12 {\boldsymbol f}({\boldsymbol x}, {\boldsymbol x}') \cdot {\boldsymbol \eta} {{\mathrm d\!} V_{{\boldsymbol x}'}} \\
...
&= \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} {\boldsymbol \eta}^{\mathrm T} \bigl( c({\boldsymbol x}, {\boldsymbol \xi}) \xi^{-2} {\boldsymbol \xi} \otimes {\boldsymbol \xi} \bigr)^{\mathrm T} {\boldsymbol \eta} {{\mathrm d\!} V_{{\boldsymbol x}'}} \\
&= \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) \xi^{-2} {\boldsymbol \eta}^{\mathrm T} ({\boldsymbol \xi} \otimes {\boldsymbol \xi}) {\boldsymbol \eta} {{\mathrm d\!} V_{{\boldsymbol x}'}}
\end{aligned}
$$
即只需要计算得到键的形变，即可通过上式计算出当前状态下 BPD 模型的弹性能密度。

## 能量意义下的局部近似 “Local” Approximation of the Energy

1

考虑局部区域的等应变假设（an infinitesimal homogeneous transformation）${\boldsymbol \varepsilon}({\boldsymbol x}) \simeq {\boldsymbol \varepsilon}({\boldsymbol x}') = \overline{\boldsymbol \varepsilon}$，同时带入键的路径积分 $\int_{{\boldsymbol \xi}} {\boldsymbol \varepsilon} {\mathrm d\!} s = \overline{\boldsymbol \varepsilon} {\boldsymbol \xi} = {\boldsymbol \eta}, \forall {\boldsymbol x}' \in {\mathcal H_\delta({\boldsymbol x})}$，可以得到由等效刚度 ${\mathbf K}_{\mathrm{pd}}({\boldsymbol x})$ 和应变场 ${\boldsymbol \varepsilon}({\boldsymbol x})$ 来表示的 BPD 模型下应变能密度的形式，即
$$
\begin{aligned}
W_{\mathrm{pd}}({\boldsymbol x})
&= \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) \bigl( \overline{\boldsymbol \varepsilon} {\boldsymbol \xi} \bigr)^{\mathrm T} {\boldsymbol \xi} \otimes {\boldsymbol \xi} \bigl( \overline{\boldsymbol \varepsilon} {\boldsymbol \xi} \bigr) {{\mathrm d\!} V_{{\boldsymbol x}'}} \\
&= \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) \bigl( {\boldsymbol \xi}^{\mathrm T} \overline{\boldsymbol \varepsilon} \bigr) {\boldsymbol \xi} \otimes {\boldsymbol \xi} \bigl( \overline{\boldsymbol \varepsilon} {\boldsymbol \xi} \bigr) {{\mathrm d\!} V_{{\boldsymbol x}'}}
\end{aligned}
$$
注意到由于最终的结果实际上是标量，所以进一步整理中间的式子有
$$
\begin{aligned}
W_{\mathrm{pd}}({\boldsymbol x})
&= \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) \bigl( {\boldsymbol \xi}^{\mathrm T} \overline{\boldsymbol \varepsilon} \bigr) {\boldsymbol \xi} \otimes {\boldsymbol \xi} \bigl( \overline{\boldsymbol \varepsilon} {\boldsymbol \xi} \bigr) {{\mathrm d\!} V_{{\boldsymbol x}'}}
&& \\
&= \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) \bigl( {\boldsymbol \xi}^{\mathrm T} \overline{\boldsymbol \varepsilon} \bigr) \bigl( {\boldsymbol \xi} \otimes {\boldsymbol \xi} \bigr) \overline{\boldsymbol \varepsilon} {\boldsymbol \xi} {{\mathrm d\!} V_{{\boldsymbol x}'}}
&&\quad \bigl( \xi_i \overline\varepsilon_{ij} (\xi_j \xi_k) \overline\varepsilon_{kl} \xi_l \bigr) \\
&= \frac14 \int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) \overline{\boldsymbol \varepsilon} : \bigl( {\boldsymbol \xi} \otimes {\boldsymbol \xi} \otimes {\boldsymbol \xi} \otimes {\boldsymbol \xi} \bigr) : \overline{\boldsymbol \varepsilon} {{\mathrm d\!} V_{{\boldsymbol x}'}}
&&\quad \bigl( \overline\varepsilon_{ij} (\xi_i \xi_j \xi_k \xi_l) \overline\varepsilon_{kl} \bigr) \\
&= \frac12 \overline{\boldsymbol \varepsilon} : \frac12\int_{\mathcal H_\delta({\boldsymbol x})} c({\boldsymbol x}, {\boldsymbol \xi}) {\boldsymbol \xi} \otimes {\boldsymbol \xi} \otimes {\boldsymbol \xi} \otimes {\boldsymbol \xi} {{\mathrm d\!} V_{{\boldsymbol x}'}} : \overline{\boldsymbol \varepsilon} \\
&= \frac12 {\boldsymbol \varepsilon}({\boldsymbol x}) : {\mathbf K}_{\mathrm{pd}}({\boldsymbol x}) : {\boldsymbol \varepsilon}({\boldsymbol x})
\end{aligned}
$$
这里，假设不同的物理模型下依旧保持应变能密度的相等，所以理论上应当有 $W_{\mathrm{pd}}({\boldsymbol x}) = W_{\mathrm{ccm}}({\boldsymbol x})$。然而实际上两者是有区别的，所以我们需要定义边界修正量来弥补由于模型自身的问题而产生的误差。
...
$$
\begin{aligned}
{\mathbf K}_{\mathrm{bond}}({\boldsymbol \xi}) &= \frac12 c({\boldsymbol x}, {\boldsymbol \xi}) {\boldsymbol \xi} \otimes {\boldsymbol \xi} \otimes {\boldsymbol \xi} \otimes {\boldsymbol \xi}\\
...
= \frac12 c({\boldsymbol x}, {\boldsymbol \xi}) {\boldsymbol \eta}^{\mathrm T} ({\boldsymbol \xi} \otimes {\boldsymbol \xi}) {\boldsymbol \eta}
